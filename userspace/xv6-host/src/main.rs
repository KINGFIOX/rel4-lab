...
use consts::{
    DISK_SERVER_ELF, DISK_SERVER_PID, FAULT_UNKNOWN_SYSCALL, FAULT_VM_FAULT, FS_OP_INIT,
    FS_SERVER_ELF, FS_SERVER_PID, LABEL_IRQ_ISSUE_IRQ_HANDLER, LABEL_IRQ_SET_NOTIFICATION,
    LABEL_RISCV_PAGE_MAP, VIRTIO_BLK_SECTOR_SIZE, XV6_ABI_VERSION, XV6_DISK_COMPLETION_NTFN_CPTR,
    XV6_DISK_COMPLETION_RING_VADDR, XV6_DISK_ENDPOINT_CPTR, XV6_DISK_SHARED_BUFFER_VADDR,
...
static mut PROCS: [Child; MAX_PROCS] = [Child::empty(); MAX_PROCS];
...
fn run(bi_ptr: *const BootInfo) -> ! {
    ...
    let procs = unsafe { &mut *core::ptr::addr_of_mut!(PROCS) };
    ...
    if !xv6::retain_initial_cwd(&procs[0]) {
        log("xv6-host: failed to retain initial cwd\n");
        halt_loop();
    }
    ...
    loop {
        ...
        if label == 0 {
            xv6::tick();
            xv6::pump_console_waiters(&mut alloc, procs);
    ...
    spawn_service_server(
        ...
        &[
            ...
            (
                ...
                cap_rights(false, false, false, true),
                consts::XV6_DISK_COMPLETION_BADGE,
            ),
        ],
        &[
