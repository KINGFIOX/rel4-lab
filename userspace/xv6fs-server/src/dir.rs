...
use sel4_user::{LogBytes, info, read_u16, write_u16};
use xv6_abi::{DIRENT_SIZE, DIRSIZ, FS_BLOCK_SIZE, XV6_FS_ROOT_INUM, Xv6FileType};

use crate::block::{read_disk_block, shared_block, with_shared_block_mut, write_disk_block};
...
pub(crate) fn find_dir_entry(dir: &Dinode, name: &[u8]) -> Option<DirEntryLoc> {
    ...
    while block_index * entries_per_block < total {
        ...
        while entry < entries {
            ...
            if inum != 0 && name_len == name.len() && dirent_name_eq(block, name, off + 2) {
...
pub(crate) fn add_dir_entry_to_inode(
    ...
    if dir.typ != Xv6FileType::Directory.raw() as i16 || find_dir_entry(dir, name).is_some() {
        return false;
    }
...
pub(crate) fn is_dir_empty(dir: &Dinode) -> bool {
    if dir.typ != Xv6FileType::Directory.raw() as i16 {
        return false;
    }
    let total = dir.size as usize / DIRENT_SIZE;
    let entries_per_block = FS_BLOCK_SIZE / DIRENT_SIZE;
...
fn dirent_name_eq(block: &[u8], name: &[u8], name_offset: usize) -> bool {
    let mut i = 0usize;
    while i < name.len() {
...
pub(crate) fn lookup_parent_from(
    ...
    } else if slash == 1 && path.first() == Some(&b'/') {
        read_inode(XV6_FS_ROOT_INUM).map(|inode| (XV6_FS_ROOT_INUM, inode))?
    } else {
        lookup_path_from(cwd_inum, &path[..slash])?
    };
    if parent.1.typ != Xv6FileType::Directory.raw() as i16 {
        return None;
    }
...
pub(crate) fn create_node_from(
    ...
    if let Some(existing) = find_dir_entry(&parent, name) {
        ...
        if typ == Xv6FileType::File.raw()
            && (existing_inode.typ == Xv6FileType::File.raw() as i16
                || existing_inode.typ == Xv6FileType::Device.raw() as i16)
        {
    ...
    if typ == Xv6FileType::Directory.raw() {
