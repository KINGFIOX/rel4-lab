...
use crate::block::{
    copy_shared_block_to_ptr, read_disk_block, read_disk_block_async, shared_block,
    with_shared_block_mut, write_disk_block, write_disk_block_async,
};
use crate::types::{BPB, DINODE_SIZE, DINODES_PER_BLOCK, Dinode, FS_MAX_TRACKED_INODES, FS_STATE};

struct OpenRefs {
    refs: UnsafeCell<[u16; FS_MAX_TRACKED_INODES]>,
}

// xv6fs-server handles inode reference updates from a single cooperative
// request loop, so this table is serialized by server control flow.
unsafe impl Sync for OpenRefs {}

impl OpenRefs {
    const fn new() -> Self {
        Self {
            refs: UnsafeCell::new([0; FS_MAX_TRACKED_INODES]),
        }
    }

    fn retain(&self, index: usize) -> bool {
        let refs = unsafe { &mut *self.refs.get() };
        if refs[index] == u16::MAX {
            return false;
        }
        refs[index] = refs[index].saturating_add(1);
        true
    }
    ...
    fn count(&self, index: usize) -> u16 {
        unsafe { (&*self.refs.get())[index] }
    }
}

static OPEN_REFS: OpenRefs = OpenRefs::new();
...
pub(crate) async fn read_inode_async(inum: u32) -> Option<Dinode> {
    let state = FS_STATE.get();
    if !state.ready || inum >= state.superblock.ninodes || inum == 0 {
        return None;
    }
    let blockno = inum / DINODES_PER_BLOCK + state.superblock.inodestart;
    let offset = ((inum % DINODES_PER_BLOCK) as usize) * DINODE_SIZE;
    if !read_disk_block_async(blockno).await {
        return None;
    }
    fence(Ordering::SeqCst);
    let block = shared_block();
...
pub(crate) async fn write_inode_async(inum: u32, inode: &Dinode) -> bool {
    let state = FS_STATE.get();
    if !state.ready || inum >= state.superblock.ninodes || inum == 0 {
        return false;
    }
    ...
    fence(Ordering::SeqCst);
    write_disk_block_async(blockno).await
}
...
pub(crate) fn retain_inode(inum: u32) -> bool {
    if let Some(index) = inode_ref_index(inum) {
        return OPEN_REFS.retain(index);
    }
    false
}

pub(crate) fn release_inode(inum: u32) -> bool {
    if let Some(index) = inode_ref_index(inum) {
        return OPEN_REFS.release(index);
    }
    false
}
...
pub(crate) fn truncate_inode(inode: &mut Dinode) -> bool {
    ...
    if indirect != 0 {
        ...
        copy_shared_block_to_ptr(indirect_block.as_mut_ptr());
        i = 0;
...
pub(crate) async fn write_inode_data_async(
    inum: u32,
    inode: &mut Dinode,
    offset: usize,
    src: &[u8],
    ...
    while written < total {
        ...
        let Some(blockno) = bmap_alloc_async(inode, file_block_index).await else {
            break;
        };
        if !read_disk_block_async(blockno).await {
            break;
        ...
        fence(Ordering::SeqCst);
        if !write_disk_block_async(blockno).await {
            break;
        }
        written += n;
    }
    ...
    write_inode_async(inum, inode).await.then_some(written)
}
...
pub(crate) fn bmap_alloc(inode: &mut Dinode, file_block_index: usize) -> Option<u32> {
    ...
    fence(Ordering::SeqCst);
    with_shared_block_mut(|block| write_u32(block, indirect_index * 4, new_block));
    fence(Ordering::SeqCst);
...
async fn bmap_alloc_async(inode: &mut Dinode, file_block_index: usize) -> Option<u32> {
    if file_block_index < XV6_FS_NDIRECT {
        if inode.addrs[file_block_index] == 0 {
            inode.addrs[file_block_index] = alloc_block_async().await?;
        }
        return Some(inode.addrs[file_block_index]);
    }
    ...
    if inode.addrs[XV6_FS_NDIRECT] == 0 {
        inode.addrs[XV6_FS_NDIRECT] = alloc_block_async().await?;
    }
    let indirect = inode.addrs[XV6_FS_NDIRECT];
    if !read_disk_block_async(indirect).await {
        return None;
    }
    fence(Ordering::SeqCst);
    let existing = read_u32(shared_block(), indirect_index * 4);
    ...
    let new_block = alloc_block_async().await?;
    if !read_disk_block_async(indirect).await {
        let _ = free_block_async(new_block).await;
        return None;
    }
    fence(Ordering::SeqCst);
    with_shared_block_mut(|block| write_u32(block, indirect_index * 4, new_block));
    fence(Ordering::SeqCst);
    if !write_disk_block_async(indirect).await {
        let _ = free_block_async(new_block).await;
        return None;
    }
    Some(new_block)
...
async fn alloc_block_async() -> Option<u32> {
    let state = FS_STATE.get();
    if !state.ready {
        return None;
    }
    ...
    while base < state.superblock.size {
        let bitmap = bitmap_blockno(base);
        if !read_disk_block_async(bitmap).await {
            return None;
        }
        fence(Ordering::SeqCst);
        let mut bit = 0u32;
        while bit < BPB && base + bit < state.superblock.size {
            ...
            if shared_block()[byte] & mask == 0 {
                ...
                fence(Ordering::SeqCst);
                if !write_disk_block_async(bitmap).await {
                    return None;
                }
                let blockno = base + bit;
                return zero_block_async(blockno).await.then_some(blockno);
            }
            bit += 1;
        }
        base += BPB;
...
async fn free_block_async(blockno: u32) -> bool {
    let state = FS_STATE.get();
    if !state.ready || blockno >= state.superblock.size {
        return false;
    }
    ...
    fence(Ordering::SeqCst);
    write_disk_block_async(bitmap).await
}
...
async fn zero_block_async(blockno: u32) -> bool {
    with_shared_block_mut(|block| {
        let mut i = 0usize;
        while i < FS_BLOCK_SIZE {
            block[i] = 0;
