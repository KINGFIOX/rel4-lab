...
fn main() {
    ...
    println!(
        "cargo:rustc-link-arg-bin=xv6fs-server=-T{}",
        linker_script.display()
    );
    println!("cargo:rustc-link-arg-bin=xv6fs-server=--no-relax");
    println!("cargo:rustc-link-arg-bin=xv6fs-server=-zmax-page-size=4096");
}
...
fn linker_script_for_target(manifest_dir: &PathBuf) -> PathBuf {
    ...
    let filename = match target.as_str() {
        ...
        _ => panic!("unsupported target for xv6fs-server: {target}"),
    };
