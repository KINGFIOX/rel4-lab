...
// linux-compat handles all process memory operations from the single rootserver
...
const EMPTY_PAGE_TABLE_ENTRY: PageTableEntry = PageTableEntry {
    pid: 0,
    range_base: 0,
    slot: 0,
};
...
struct ChildPageTables {
    state: UnsafeCell<PageTableState>,
}

// Page-table cap tracking is only mutated by the single linux-compat rootserver
// fault loop.
unsafe impl Sync for ChildPageTables {}

impl ChildPageTables {
    const fn new() -> Self {
        Self {
            state: UnsafeCell::new(PageTableState::new()),
        }
    }
...
static CHILD_PAGE_TABLES: ChildPageTables = ChildPageTables::new();

fn insert_page_table_entry(entries: &mut [PageTableEntry], pid: u64, range_base: u64, slot: u64) {
    ...
    warn!("linux-compat: page table registry full");
    halt_loop();
}

fn clear_page_table_entries(alloc: &mut Allocator, entries: &mut [PageTableEntry], pid: u64) {
    for entry in entries {
        if entry.pid == pid {
            alloc.delete_cap_slot(entry.slot);
            *entry = EMPTY_PAGE_TABLE_ENTRY;
        }
    }
}
...
pub(crate) fn create_child_from_untyped(
    ...
    ensure_page_table_path(alloc, pid, vspace, CHILD_IPC_BUFFER);
...
pub(crate) fn load_elf(alloc: &mut Allocator, child: &mut TaskStruct, elf: &[u8]) {
    if !elf_image_valid(elf) {
        warn!("linux-compat: bad payload ELF");
        halt_loop();
    }
    ...
    for i in 0..phnum {
        ...
        if off + 56 > elf.len() {
            warn!("linux-compat: truncated program headers");
            halt_loop();
        }
        ...
        if p_offset.saturating_add(p_filesz) > elf.len() {
            warn!("linux-compat: segment outside payload");
            halt_loop();
        }
        ...
        {
            warn!("linux-compat: failed to copy payload");
            halt_loop();
        }
    ...
    child.elf_phent = phentsize as u16;
    child.elf_phnum = phnum as u16;
    child.elf_phdr = elf_phdr_vaddr(elf, phoff);
    info!(
        "linux-compat: payload entry={:#x} brk={:#x} phdr={:#x}",
        entry, child.brk, child.elf_phdr
    );
}

fn elf_phdr_vaddr(elf: &[u8], phoff: usize) -> u64 {
    let phentsize = read_u16(elf, 54) as usize;
    let phnum = read_u16(elf, 56) as usize;
    let mut i = 0usize;
    while i < phnum {
        ...
        if read_u32(elf, off) == 1 {
            ...
            let p_vaddr = read_u64(elf, off + 16);
            if p_offset <= phoff as u64 {
                return p_vaddr + (phoff as u64 - p_offset);
            }
        }
        i += 1;
    }
...
pub(crate) fn frame_paddr(frame_slot: u64) -> u64 {
    ...
    if err != 0 {
        warn!("linux-compat: Page_GetAddress failed err={}", err);
        halt_loop();
    }
    reply.mrs[0]
}
...
fn register_mapping(
    ...
    let Some((slot, alias)) = CHILD_MEMORY.allocate_mapping_slot() else {
        warn!("linux-compat: mapping table full");
        halt_loop();
    };
...
pub(crate) fn read_user_context(tcb: u64) -> [u64; USER_CONTEXT_WORDS] {
    ...
    let err = msg_label(reply.info);
    if err != 0 {
        warn!("linux-compat: TCB_ReadRegisters failed err={}", err);
        halt_loop();
    }
...
fn ensure_alias_for_index(alloc: &mut Allocator, slot: usize) -> u64 {
    let mapping = CHILD_MEMORY.mapping(slot);
    if mapping.pid == 0 {
        warn!("linux-compat: alias for empty mapping");
        halt_loop();
    }
    if mapping.alias_slot == 0 {
        let alias_slot = alloc.copy_cap(mapping.frame_slot, cap_rights(false, false, true, true));
        ensure_host_page_table_path(alloc, mapping.alias_page);
...
fn zero_frame_with_temporary_alias(alloc: &mut Allocator, frame_slot: u64, alias_va: u64) {
    ...
    ensure_host_page_table_path(alloc, alias_va);
