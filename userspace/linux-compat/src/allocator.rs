...
use crate::consts::{
    DEVICE_MMIO_BASE, DEVICE_MMIO_SIZE, LABEL_CNODE_COPY, LABEL_CNODE_DELETE, LABEL_CNODE_MINT,
    LABEL_CNODE_REVOKE, LABEL_UNTYPED_RETYPE, MAX_PROCS, MAX_RECYCLED_SLOTS, OBJ_UNTYPED,
    PROCESS_UNTYPED_BITS, PROCESS_UNTYPED_PARENT_BITS, ROOT_CNODE, ROOT_CNODE_DEPTH,
};
...
// linux-compat allocates and recycles root CSpace slots from the single rootserver
...
impl Allocator {
    pub(crate) fn new(bi: &BootInfo) -> Self {
        ...
        if selected == 0 {
            warn!("linux-compat: no usable untyped");
            halt_loop();
        }
        if process_parent == 0 {
    ...
    pub(crate) fn alloc_slot(&mut self) -> u64 {
        ...
        if self.next_slot >= self.empty_end {
            warn!(
                "linux-compat: out of CSpace slots next={} end={} recycled={}",
    ...
    pub(crate) fn retype_device_4k_at(&mut self, paddr: u64) -> u64 {
        if paddr & (crate::consts::PAGE_SIZE - 1) != 0 {
            warn!("linux-compat: invalid device frame request pa={:#x}", paddr);
            halt_loop();
        }
    ...
    pub(crate) fn process_untyped(&self, proc_slot: usize) -> u64 {
        if proc_slot >= MAX_PROCS || self.process_untyped_slots[proc_slot] == 0 {
            warn!("linux-compat: invalid process untyped slot");
            halt_loop();
        }
...
fn largest_aligned_chunk_bits(cursor: u64, remaining: u64) -> u8 {
    ...
    if bits < 12 {
        warn!("linux-compat: device untyped skip lost page alignment");
        halt_loop();
    }
    bits
}
...
fn push_device_region(
    ...
    if *count >= MAX_DEVICE_REGIONS {
        warn!("linux-compat: too many device untyped regions");
        halt_loop();
    }
