...
pub(crate) fn sys_brk(alloc: &mut Allocator, child: &mut TaskStruct, addr: u64) -> i64 {
    if addr == 0 {
        return child.brk as i64;
    }
    let old = child.brk;
    if addr > CHILD_HEAP_LIMIT {
        return old as i64;
    }
    let increment = addr as i64 - old as i64;
    if sys_sbrk(alloc, child, increment, SBRK_LAZY) < 0 {
        return old as i64;
    }
    child.brk as i64
}
...
pub(crate) fn sys_mmap(
    ...
    let len = align_up(length);
    let start = if addr != 0 && flags & MAP_FIXED != 0 {
        addr
    } else if addr != 0 {
        addr
    } else {
    ...
    if start < child.heap_start && flags & MAP_FIXED == 0 {
        // Place anonymous maps at the current break.
        let mapped = sys_sbrk(alloc, child, len as i64, SBRK_EAGER);
        if mapped < 0 {
    ...
    if end > CHILD_HEAP_LIMIT {
        return -(ENOMEM as i64);
    }
    let mut page = crate::util::align_down(start);
    while page < end {
    ...
    if end > child.brk {
        child.brk = end;
        child.heap_mapped_end = child.heap_mapped_end.max(end);
    }
    start as i64
}

pub(crate) fn sys_mprotect(_child: &TaskStruct, _addr: u64, _len: u64, _prot: u32) -> i64 {
    0
}
...
pub(crate) fn sys_munmap(
    ...
    unmap_child_range(alloc, child.pid, addr, addr.saturating_add(len));
    0
}

fn sys_sbrk(alloc: &mut Allocator, child: &mut TaskStruct, increment: i64, mode: u64) -> i64 {
    let old = child.brk;
    ...
    if new_brk > old {
        ...
        if needed <= SBRK_EAGER_MAP_LIMIT {
            ...
            && !reserve_sparse_eager(child, target_end.saturating_sub(first_page))
        {
            return -1;
        }
    }
...
pub(crate) fn handle_lazy_page_fault(
    ...
    if mapping_slots_available() <= SBRK_MAPPING_HEADROOM {
        crate::util::warn!(
            "linux-compat: lazy fault mapping slots exhausted pid={} mapped={} slots={}",
            child.pid,
            mapping_count(),
    ...
    if frame_pool_available() == 0 && alloc.slots_available() <= SBRK_MAPPING_HEADROOM {
        crate::util::warn!(
            "linux-compat: lazy fault CSpace exhausted pid={} mapped={}",
            child.pid,
            mapping_count()
        );
        return false;
