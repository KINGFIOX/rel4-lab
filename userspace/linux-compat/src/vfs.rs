...
// linux-compat drives VFS client state from the single rootserver event loop.
...
pub(crate) fn start_vfs_lseek_request(
    alloc: &mut Allocator,
    child: &mut TaskStruct,
    syscall_mrs: &[u64; 64],
    file: usize,
...
pub(crate) fn start_vfs_read_request(
    ...
    let Some(file) = fd_file(child, fd) else {
        return crate::types::SyscallResult::err(EBADF);
    };
...
pub(crate) fn start_vfs_write_request(
    ...
    let request = min(len, MAX_IO_BYTES);
    if !copy_child_to_vfs_shared_buffer(alloc, child, buf, request) {
        return crate::types::SyscallResult::err(EFAULT);
    }
...
fn start_vfs_async_request(
    ...
    if ep == 0 || request_mrs.len() > 64 {
        return crate::types::SyscallResult::err(EIO);
    }
    let Some(slot) = alloc_vfs_async_request() else {
        return crate::types::SyscallResult::err(EAGAIN);
    };
    let request_id = next_vfs_request_id();
    let (reply_slot, reply_mrs) = save_child_reply(syscall_mrs);
...
pub(crate) fn resume_vfs_waiter_async(
    ...
    unsafe {
        sel4_send(
            ...
            &[
                IpcProtocol::HostToVfsAsync.raw(),
                request_id,
                file as u64,
                request as u64,
            ],
...
fn complete_vfs_async_state(
    ...
    let ok = status == IpcStatus::Ok.raw();
    let mut ret = if ok { 0 } else { -(status as i64) };
    for child in procs.iter_mut() {
        if child.pid != request.pid {
            continue;
        }
        if request.kind == VFS_ASYNC_READ && status == IpcStatus::WouldBlock.raw() {
            set_vfs_waiter(child, request, PROC_VFS_READ, request.done);
            return VfsCompletion::wait();
        }
        if request.kind == VFS_ASYNC_WRITE && status == IpcStatus::WouldBlock.raw() {
            ...
            match request.kind {
                VFS_ASYNC_OPEN => {
                    ...
                    if request.fd < MAX_FD && file < MAX_OPEN_FILES {
                        ...
                        child.fd_serial[request.fd] = msg.mrs[3] != FileKind::Device.raw() as u64;
                ...
                VFS_ASYNC_READ => {
                    ...
                    let first_request = min(request.len - request.done, PAGE_SIZE as usize);
                    if n <= first_request
                ...
                VFS_ASYNC_WRITE => {
                    let n = msg.mrs[2] as usize;
                    let first_request = min(request.len - request.done, MAX_IO_BYTES);
        ...
        } else if request.kind == VFS_ASYNC_WRITE && status == IpcStatus::BrokenPipe.raw() {
            ret = if request.done == 0 {
                -1
            } else {
                request.done as i64
...
fn save_child_reply(mrs: &[u64; 64]) -> (u64, arch::FaultReplyFrame) {
