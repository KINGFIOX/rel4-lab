#!/usr/bin/env python3
"""Audit linux-compat ELF target checks used by service loading."""
...
PREFIX = "audit-linux-compat-elf-abi"
...
def audit_build_script(errors: list[str]) -> None:
    path = ROOT_DIR / "userspace" / "linux-compat" / "build.rs"
    require_text(
        errors,
        path,
    ...
    for name in ("uart_server", "vfs_server"):
        require_text(
            errors,
            path,
...
def main() -> int:
    ...
    if errors:
        ...
        return 1
    print(f"PASS: {target.name} linux-compat ELF ABI checks")
    return 0
