...
CHECKS: tuple[Check, ...] = (
    ...
    Check(
        name="local idle restore path switches to idle TCB without FPU release",
        path="kernel/src/arch/riscv64/kernel/trap.rs",
        patterns=(
            r"fn\s+restore_idle_context\(\)\s*->\s*\*mut UserContext",
            r"switch_to_idle_thread\(\)",
            r"switch_to_kernel_vspace\(\)",
            r"prepare_for_user_restore\(\)",
        ),
        ordered=True,
        forbidden_patterns=(
            r"fn\s+restore_idle_context\(\)\s*->\s*\*mut UserContext\s*\{(?:(?!\n\}).)*fpu::",
        ),
    ),
    Check(
        name="secondary restore path resumes runnable TCB through FPU restore boundary",
        path="kernel/src/arch/riscv64/kernel/trap.rs",
        patterns=(
            r"pub\s+fn\s+restore_scheduled_or_idle\(",
            r"tcb::set_current\(Some\(next\)\)",
            r"let\s+ctx\s*=\s*next\.prepare_for_user_restore\(\)",
            r"switch_to_tcb_vspace\(next\)",
            r"restore_user_context_with_kernel_lock\(ctx,\s*kernel_lock\)",
        ),
        ordered=True,
    ),
    Check(
