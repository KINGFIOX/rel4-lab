...
def default_cmake_defs(target) -> dict[str, str]:
    values = {
...
def cache_env_overrides_differ(build_dir: Path, cache: dict[str, str], target) -> bool:
    defaults = default_cmake_defs(target)
    ...
    if "NUM_NODES" in os.environ and cache.get("NUM_NODES") != os.environ["NUM_NODES"]:
        return True
    expected_regex = os.environ.get("SEL4TEST_REGEX", defaults["LibSel4TestPrinterRegex"])
    if cache.get("LibSel4TestPrinterRegex") != expected_regex:
        return True
    qemu_dtb_name = f"{platform_from_env(target)}.dtb"
    if "QEMU_DTB" in os.environ:
        qemu_dtb = Path(os.environ["QEMU_DTB"])
        if qemu_dtb.is_file() and cache.get("QEMU_DTB") != str(qemu_dtb):
            return True
    elif (build_dir / qemu_dtb_name).is_file() and cache.get("QEMU_DTB") != str(
        build_dir / qemu_dtb_name
    ):
        return True
    return False
...
def effective_cmake_values(build_dir: Path, cache: dict[str, str], target) -> dict[str, str]:
    values = default_cmake_defs(target)
    ...
    qemu_dtb = Path(os.environ.get("QEMU_DTB", str(build_dir / f"{platform_from_env(target)}.dtb")))
    if qemu_dtb.is_file():
...
def cmake_defs(build_dir: Path, cache: dict[str, str], target) -> list[str]:
...
def audit_rust_kernel(target, rust_target: str, rust_kernel_elf: Path, env: dict[str, str]) -> None:
    ...
    log(PREFIX, "auditing user-context ABI constants...")
    run(
        [sys.executable, "tools/audit-user-context-abi.py"],
        cwd=ROOT_DIR,
        env=audit_env,
    )
    ...
    log(PREFIX, "auditing SMP remote-operation invariants...")
    run(
        [sys.executable, "tools/audit-smp-abi.py"],
        cwd=ROOT_DIR,
        env=audit_env,
    )
    ...
    log(PREFIX, "auditing VSpace ABI constants...")
    run(
        [sys.executable, "tools/audit-vspace-abi.py"],
        cwd=ROOT_DIR,
        env=audit_env,
    )
...
def ensure_sel4_configured(build_dir: Path, target) -> None:
    ...
    target.require_sel4_arch_source(PREFIX, tree_dir)
    ...
    run([str(init_build), *cmake_defs(build_dir, cache, target)], cwd=build_dir)
...
def main() -> int:
    ...
    try:
        ...
        log(
            PREFIX,
            (
                f"building Rust kernel for ARCH={target.name} target={rust_target} "
                f"(SMP={cargo_env['SMP']} NUM_NODES={cargo_env['NUM_NODES']} "
                f"ROOT_CNODE_BITS={cargo_env['KERNEL_ROOT_CNODE_SIZE_BITS']})..."
            ),
        )
        run(
