...
- `kernel_exit` appends the current runnable thread to the tail of its core runqueue and then takes the head. It runs on every trap cause, including timer interrupts. Each backend has a copy: `kernel/src/arch/riscv64/kernel/trap.rs` and `kernel/src/arch/x86_64/kernel/trap.rs`.
...
5. Keep RISC-V trap/timer behavior as the linux-compat and sel4test reference. The x86_64 backend has a matching `kernel_exit` plus `syscall`/`sysret` and a single-core LAPIC timer; do not add timeslice accounting there either.
